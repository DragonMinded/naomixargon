...
int msg_centipede (int n, int msg, int z) {
	...
	switch (msg) {
		case msg_update:
			...
			if (randomrange(30)==0) seekplayer (n,&pobj->xd,0);
				...
				if (pobj->xl<37) {
					for (c=0; c<7; c++) {
						addobj (obj_centexpl,pobj->x+((pobj->xd>0)?pobj->xl-24:16),
							pobj->y+5,randomrange(7)-3,randomrange(4)-10);
						};
...
int msg_alien (int n, int msg, int z) {
	...
	switch (msg) {
		case msg_update:
			...
			if (randomrange(20)==0) pobj->xd=(pobj->xd>0)?0:pobj->substate;
			if (randomrange(15)==0) {
				seekplayer (n,&pobj->xd,&pobj->yd);
				pobj->xd*=4;
				};
			...
			switch (pobj->state) {
				case 0:
					if ((pobj->counter>=((pobj->xd<0)?0:12))&&
						...
						if ((!crawl (n,pobj->xd,0))||(randomrange(40)==0))
							pobj->xd=-pobj->xd;
						};
					if (randomrange(50)==0) {
						if (pobj->xd<0) {
...
int msg_xargbot (int n, int msg, int z) {
	...
	switch (msg) {
		case msg_update:
			...
			if ((randomrange(((pobj->state!=2)?50:40))==0)&&(pobj->xd!=0)) {
				addobj (obj_bullet,pobj->x-4+(pobj->xd>0)
...
int msg_eel (int n, int msg, int z) {
	...
	switch (msg) {
		case msg_update:
			if (++pobj->counter>=10) pobj->counter=0;
			if (randomrange(20)==0) {
				pobj->xd=randomrange(3)*4-4;
				if (pobj->xd==0) pobj->yd=randomrange(2)*4-2;
				else pobj->yd=randomrange(3)*2-2;
				};
	...
	switch (msg) {
		case msg_update:
			...
			if (pobj->substate>=50) {				// Xargon destroyed
				...
				explode1 (pobj->x+randomrange(54),pobj->y+randomrange(60),1,1);
				explode1 (pobj->x+randomrange(54),pobj->y+randomrange(60),1,0);
				addobj (obj_fireexpl,pobj->x+randomrange(60)-15,
					pobj->y+randomrange(68)-15,0,0);
				addobj (obj_hithero,pobj->x+randomrange(60)-12,
					pobj->y+randomrange(68)-17,0,0);
				if (pobj->statecount==0) {
...
//					if (!invcount(inv_invin)) addinv (inv_invin);
			...
			if ((randomrange(20)==0)&&(pobj->info1==0)&&(pobj->yd==0)) {
				pobj->info1=15;
		...
		case msg_touch: if (pobj->substate>=50) return (0);
			...
			if (kindflags[objs[z].objkind]&f_weapon) {
				...
				explode1 (pobj->x+randomrange(55),pobj->y+randomrange(60),2,1);
				explode1 (pobj->x+randomrange(55),pobj->y+randomrange(60),3,1);
				addobj (obj_fireexpl,pobj->x+randomrange(44),pobj->y+randomrange(52),0,0);
				addobj (obj_fireexpl,pobj->x+randomrange(44),pobj->y+randomrange(52),0,0);
				killobj (z);
				}; return (1);
		}; return (0);
